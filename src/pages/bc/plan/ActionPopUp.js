...
import { Button, Card, Dialog, FormLabel, Checkbox, Tooltip, IconButton, FormControlLabel, DialogActions, DialogContent, DialogTitle, Grid, Stack, TextField } from '@mui/material';
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting, resetForm, setErrors }) => {
            try {
                ...
                // item.append('plan_id', displayModel.order_id);
                let item = {};
                item.action_id = displayModel.action_id;
                item.plan_id = Number(data.id);
                ...
                const res = await apiService.postMethod(`/order/plan/actions`, item).catch(error => {
                            ...
                            <Grid item xs={12} md={12}>
                                <Card sx={{ p: 3 }}>
                                    <Stack spacing={3}>
                                        ...
                                        {(displayModel.action_id === 3 || displayModel.action_id === 6 || displayModel.action_id === 8) && (
                                            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 3, sm: 2 }}>
                                                <WidInput
                                                    type="description"
                                                    regex="required"
                                        ...
                                        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 3, sm: 2 }}>
                                            ...
                                            >
                                                <FormControlLabel value="Auto" disabled={isEdit} checked={segView === 1} control={<Radio />} label="Auto" onClick={() => setSegView(1)} />
                                                <FormControlLabel value="Manual" disabled={isEdit} checked={segView === 2} control={<Radio />} label="Manual" onClick={() => setSegView(2)} />
                                            </RadioGroup>
                                        </Stack>
                                    </Stack>
                                </Card>
