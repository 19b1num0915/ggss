...
export default function Manual({segment, row, isEdit, displayModel, doAction}) {
  ...
  const handleUpload = async (file) => {
    ...
    data.append('order_id', row.order_id);
    data.append('file', file);
    const response = await apiService.postMethod('/segment/upload', data);
    ...
    <Page title="Сегмент">
      <Grid item xs={12} md={12}>
          <Stack spacing={1}>
            ...
            {((!isEdit &&  doAction !== 1) || displayModel?.action_id == 10) && (
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 3, sm: 2 }}>
