...
export default function PopUp({ popUpName, doAction, confirmModal, data, isEdit, popAction }) {
  ...
  const ValidationSchema = Yup.object().shape({
    sms: Yup.string().required('SMS Required'),
    descr: Yup.string().required('Нэмэлт тайлбар Required'),
  });
  ...
  const showPopUp = () => {
    if (openPopNested) {
      return (
        <PopUpNested
          popUpName={'Сегмент сонгох'}
          doAction={doActionNested}
          confirmModal={openPopNested}
          popAction={(check) => popActionNested(check)}
        />
      );
    }
  };
              ...
              <Grid item xs={12} md={12}>
                <Card sx={{ p: 3 }}>
                  <Stack spacing={3}>
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 3, sm: 2 }}>
                      {segView === 2 ? <Manual doAction={doAction} segment={(data) => segmentChanged(data)} /> : ''}
                    </Stack>
                  </Stack>
                </Card>
              </Grid>
