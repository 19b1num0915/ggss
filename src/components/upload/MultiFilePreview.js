...
export default function MultiFilePreview({ showPreview, files, onRemove, onRemoveAll, previewFile }) {
  useEffect(() => {}, [files, previewFile]);
  const hasFile = files?.length > 0;
  const hasPreview = previewFile?.length > 0;
      ...
      return (
        <Stack direction="row" justifyContent="flex-end" spacing={1.5}>
          <Button color="inherit" size="small" onClick={onRemoveAll}>
            Оруулсан бүх бичиг баримтыг устгах
          </Button>
        </Stack>
      );
    } else if (hasFile) {
          ...
          {previewFile?.map((file, index) => {
            if (showPreview) {
              return (
                ...
                >
                  <Image alt="preview" src={`http://103.153.141.93:9090/images/${file.IMAGE_PATH}`} ratio="1/1" />
                  <IconButton
                    size="small"
